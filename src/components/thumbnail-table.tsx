...
export function ThumbnailTable({
  ...
  const [popup, setPopup] = useState<{
    ...
    title: string;
    imageUrl: string;
  }>({ open: false, title: "", imageUrl: "" });
  const [hoveredImage, setHoveredImage] = useState<string | null>(null);
  const [copiedCell, setCopiedCell] = useState<string | null>(null);

  const allSelected =
    thumbnails.length > 0 && selectedRows.size === thumbnails.length;

  const toggleAllRows = () => {
    if (allSelected) {
      setSelectedRows(new Set());
    } else {
      setSelectedRows(new Set(thumbnails.map((t) => t.id)));
    }
  };
  ...
  const maxVariants = thumbnails.reduce(
    (max, t) => Math.max(max, t.variants?.length ?? 0),
    0
  );

  const deleteThumbnails = async (ids: string[]) => {
    const count = ids.length;
    const confirmed = window.confirm(
      `Are you sure you want to delete ${count} thumbnail${count > 1 ? "s" : ""}? This action cannot be undone.`
    );
    if (!confirmed) return;
  ...
  const downloadAll = (thumb: Thumbnail) => {
    thumb.variants?.forEach((v, i) => {
      downloadImage(v.variant_url, `variant_${i + 1}.png`);
    });
    toast.success(`Downloaded ${thumb.variants?.length ?? 0} variant(s)`);
  };

  const downloadSelected = () => {
    const selected = thumbnails.filter((t) => selectedRows.has(t.id));
    selected.forEach((t) => downloadAll(t));
  };
        ...
        <Table>
          ...
          <TableBody>
            {thumbnails.length === 0 && (
              <TableRow>
                <TableCell
                  colSpan={Math.max(maxVariants, 1) + (isAdmin ? 6 : 5)}
                  className="text-center text-muted-foreground py-12"
                >
                  No thumbnails yet.{" "}
                  <Link href="/generate" className="text-primary underline underline-offset-4 hover:text-primary/80">
                    Go to Generate
                  </Link>{" "}
                  to create one.
                </TableCell>
            ...
            {thumbnails.map((thumb) => (
              <TableRow key={thumb.id} className="border-b border-border/30">
                <TableCell>
                  <Checkbox
                    checked={selectedRows.has(thumb.id)}
                    onCheckedChange={() => toggleRow(thumb.id)}
                  />
                </TableCell>
                <TableCell>
                  <div
                ...
                {isAdmin && (
                  <TableCell className="text-sm text-muted-foreground">
                    {thumb.profile?.display_name || thumb.profile?.email}
                  </TableCell>
                )}
                {Array.from(
                  { length: Math.max(maxVariants, 1) },
                  (_, i) => {
                    ...
                    return (
                      <TableCell key={i}>
                        {variant ? (
                          <div
                            className="group relative cursor-pointer"
                            onMouseEnter={() => setHoveredImage(cellId)}
                            onMouseLeave={() => setHoveredImage(null)}
                            onClick={() =>
                              setPopup({
                                open: true,
                                title: `Variant ${i + 1}`,
                                imageUrl: variant.variant_url,
                              })
                            }
                          >
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            ...
                            <button
                              ...
                              className="absolute right-0 top-0 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-bl bg-background/80"
                              onClick={(e) => {
                                e.stopPropagation();
                                copyImage(variant.variant_url, cellId);
                              }}
                            >
                              {copiedCell === cellId ? (
                ...
                <TableCell>
                  <Button
                    ...
                    onClick={() => deleteThumbnails([thumb.id])}
